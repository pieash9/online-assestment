...
import { Prisma, PrismaClient, QuestionType, UserRole } from "@prisma/client";
import { createMySqlAdapter } from "../src/lib/prisma-adapter";
...
const prisma = new PrismaClient({
  adapter: createMySqlAdapter(databaseUrl)
});
...
async function ensureDatabaseSchemaReady() {
  try {
    ...
    if (
      ...
      throw new Error(
        "Database tables do not exist yet. Run `yarn prisma:migrate` first, then run `yarn prisma:seed`."
      );
    }
